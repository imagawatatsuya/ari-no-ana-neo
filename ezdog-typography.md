...
- 字下げ: 読者設定に応じて `jisage-core` が表示用コピーへ全角スペース `　` を挿入（CSS text-indent は不使用）
- 投稿本文は原文を保存し、投稿者の意図（自動字下げなし／自動字下げあり）を別メタデータで保持する。旧投稿・互換入力は `raw`（原文どおり）として扱う
- 読者設定は「自動字下げなし／自動字下げあり／投稿者設定に従う」の3択で、未設定時は投稿者設定に従う。明示した読者設定を最終優先とする
- 表示用コピーでは全角空白一字を通常の字下げマーカーとして読者設定に応じて整理し、二字以上の手動字下げと脚注本文は自動字下げ処理の対象外
- ドロップキャップ: **本项目では不使用**
